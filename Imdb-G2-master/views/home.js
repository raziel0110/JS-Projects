$(document).ready(() => {
  ...
  if (authToken !== null) {
    ...
    topRatedMovies.parentNode.insertBefore(
      displayAddBtn,
      topRatedMovies.nextSibling
    );
    ...
    searchList.fetchMovieList(filterList, input).then(item => {
      ...
      if (item.results == "") {
        ...
        for (let i = 0; i < item.results.length; i++) {
          ...
          ancor_element.setAttribute(
            "href",
            "../pages/movieDetails.html?postId=" + item.results[i]._id
          );
  ...
  movieList
    ...
    .then(() => {
      ...
      for (let i = 0; i < movieList.items.length; i++) {
        ...
        ancor_element.setAttribute(
          "href",
          "../pages/movieDetails.html?postId=" + movieList.items[i]._id
        );
        movieElement.setAttribute("class", "movie-element");
        moviesGlobalContainer.appendChild(movieElement);
        const posterContainer = document.createElement("div");
      ...
      .then(() => {
        ...
        for (let i = 0; i < movieList.items.length; i++) {
          ...
          imdbRating.innerHTML =
            "Rating &nbsp;" + movieList.items[i].imdbRating;
          imdbRating.setAttribute("class", "imdb-rating");
          posterContainer.appendChild(imdbRating);
          const movieTitle = document.createElement("p");
          movieTitle.setAttribute("class", "movie-title");
